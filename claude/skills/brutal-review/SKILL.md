---
description: Perform a ruthless, multi-perspective code review of the current branch (vs main).
...
Perform a ruthless, brutal, in-depth, extremely critical code review of the current branch (all changes vs `main`).
...
Get the full commit history and diff for this branch. Read every line carefully.

<!-- For git diff use three dots (...) since it diffs against the merge base rather than the current tip of main -->
...
5. **Write the CONTEXT BLOCK to a temporary file**:
   ...
   First, get the commit ID to use in the filename:
...
- Does the code actually do what the documentation, comments, and commit messages claim?
...
### Perspective 5: Security (use for `[PERSPECTIVE-SPECIFIC INSTRUCTIONS]`)

This subagent takes the perspective of a paranoid security auditor who assumes every input is adversarial, deeply considering:

**Trust Boundaries & Input**

- Is all external input validated, sanitized, and bounded?
- Are there injection vectors (SQL, command, path traversal, template, header)?
- Are auth checks enforced at every entry point, with no bypass logic?
- Are there TOCTOU (time-of-check-time-of-use) gaps in permission checks?

**Data & Secrets**

- Are secrets handled correctly—never logged, never hardcoded, rotated properly?
- Could error messages, logs, or API responses leak sensitive data?
- Are cryptographic primitives used correctly (no custom crypto, proper randomness, correct modes)?

**Attack Surface & Blast Radius**

- Does the code operate at minimum required privilege?
- If this component is compromised, what is the blast radius?
- Are new dependencies pinned, actively maintained, and worth the added trust surface?
- Are file and network operations safe from symlink attacks, path traversal, and open redirects?
